...
class VegaDivergence(Strategy):
    ...
    def next(self):
        ...
        sma10 = float(self.sma10[-1])
        atr = float(self.atr[-1])
        r_spx_20 = float(self.r_spx_20[-1])
        r_vix_20 = float(self.r_vix_20[-1])
        z_vix_20 = float(self.z_r_vix_20[-1])
        vix_level = float(self.vix_proxy_level[-1])
        ...
        try:
            ...
            print(f"🌙 [Moon Dev Debug] Print formatting error at bar {i}: {e}")
        ...
        if divergence and not self.position:
            ...
            sl_price = float(close + stop_dist)  # short stop (absolute price level)
            tp_price = float(close - stop_dist * self.tp_mult)  # profit target (absolute price level)
            ...
            # Enforce unit-based sizing: positive whole number
            size = int(round(size))
            if size <= 0:
            ...
            self.entry_bar = i
            self.entry_side_sign = float(np.sign(close - sma10))
...
stats = bt.run()
print("🌙 [Moon Dev] Backtest complete. Summary stats below: ✨")
print(stats)
try:
    strat_obj = getattr(stats, '_strategy', None)
    if strat_obj is not None:
        print("🌙 [Moon Dev] Strategy instance details: ✨")
        print(strat_obj)
    else:
        print("🌙 [Moon Dev] No _strategy attribute available in stats. 🌑")
except Exception as e:
    print(f"🌙 [Moon Dev] Error printing strategy instance: {e}")