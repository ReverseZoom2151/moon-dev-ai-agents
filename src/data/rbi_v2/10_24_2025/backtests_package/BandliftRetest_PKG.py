...
class BandliftRetest(Strategy):
    ...
    def init(self):
        ...
        # Compute SMA baseline directly from Close
        def _sma(c, period):
            return talib.SMA(c, timeperiod=int(period))

        self.baseline = self.I(_sma, self.data.Close, self.sma_period)

        # Slope of baseline using LINEARREG_SLOPE on the SMA(20)
        def _slope_of_sma(c, sma_p, slope_p):
            sma_vals = talib.SMA(c, timeperiod=int(sma_p))
            return talib.LINEARREG_SLOPE(sma_vals, timeperiod=int(slope_p))

        self.baseline_slope = self.I(_slope_of_sma, self.data.Close, self.sma_period, 5)
