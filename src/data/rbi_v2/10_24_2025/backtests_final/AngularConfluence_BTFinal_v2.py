...
def moon_print(*args, **kwargs):
    # Safe print that strips unsupported unicode for Windows consoles
    try:
        print(*args, **kwargs)
...
class AngularConfluence(Strategy):
    ...
    def next(self):
        ...
        # Moon Dev Debugging
        moon_print(f"[🌙 AngularConfluence] Bar {i} | Price: {c:.2f} | SMA5: {sma:.2f} | ATR14: {atr:.2f} | Tol: {tol:.4f} 🚀")
        ...
        if self.position:
            if self.position.is_long:
                ...
                if np.isfinite(self.r1x1[-1]) and abs(self.sma5[-1] - self.r1x1[-1]) <= tol:
                    moon_print("✨ [Moon Dev] SMA met rising 1x1 confluence. Locking lunar profits on LONG! 🌕")
                    self.position.close()
                ...
                if self.entry_price is not None and self.initial_stop is not None:
                    ...
                    if (c - self.entry_price) >= one_r:
                        ...
                        if c <= trail:
                            moon_print("🛡️ [Moon Dev] Trailing stop hit on LONG. Banking stardust gains! 🌠")
                            self.position.close()
                            self.entry_price = self.entry_bar = self.initial_stop = self.r_multiple = None
                            self.active_angle_side = None
                            return
                ...
                if self.entry_bar is not None and (i - self.entry_bar) >= self.time_stop_bars:
                    moon_print("⏳ [Moon Dev] Time stop: Exiting LONG due to time decay in orbit. 🛰️")
                    self.position.close()
                    self.entry_price = self.entry_bar = self.initial_stop = self.r_multiple = None
                    self.active_angle_side = None
                    return
                ...
                if np.isfinite(self.f1x1[-1]) and abs(self.sma5[-1] - self.f1x1[-1]) <= tol:
                    moon_print("✨ [Moon Dev] SMA met falling 1x1 confluence. Locking lunar profits on SHORT! 🌘")
                    self.position.close()
                    self.entry_price = self.entry_bar = self.initial_stop = self.r_multiple = None
                    self.active_angle_side = None
                    return
                ...
                if self.entry_price is not None and self.initial_stop is not None:
                    ...
                    if (self.entry_price - c) >= one_r:
                        ...
                        if c >= trail:
                            moon_print("🛡️ [Moon Dev] Trailing stop hit on SHORT. Banking stardust gains! 🌠")
                            self.position.close()
                            self.entry_price = self.entry_bar = self.initial_stop = self.r_multiple = None
                            self.active_angle_side = None
                            return
                ...
                # Time stop
                if self.entry_bar is not None and (i - self.entry_bar) >= self.time_stop_bars:
                    moon_print("⏳ [Moon Dev] Time stop: Exiting SHORT due to time decay in orbit. 🛰️")
                    self.position.close()
                    self.entry_price = self.entry_bar = self.initial_stop = self.r_multiple = None
                    self.active_angle_side = None
                    return
        ...
        if trend_up and confluence_long and atr_ok and trigger_long:
            ...
            if rr >= self.min_rr:
                ...
                moon_print(f"🚀 [Moon Dev LONG] Trend up + confluence at {rising_tag}. RR={rr:.2f} | Size={size} | SL={stop_long:.2f} 🌙")
                ...
                moon_print(f"🌓 [Moon Dev] LONG rejected: RR {rr:.2f} below {self.min_rr}. Waiting for better lunar alignment.")
        ...
        if trend_down and confluence_short and atr_ok and trigger_short:
            ...
            if rr >= self.min_rr:
                ...
                moon_print(f"🚀 [Moon Dev SHORT] Trend down + confluence at {falling_tag}. RR={rr:.2f} | Size={size} | SL={stop_short:.2f} 🌑")
                ...
                moon_print(f"🌗 [Moon Dev] SHORT rejected: RR {rr:.2f} below {self.min_rr}. Awaiting better eclipse.")

        # Otherwise no trade
        moon_print("🛰️ [Moon Dev] Standing by... No confluence or RR not met this bar.")


def _resolve_data_path(preferred_path: str, filename_hint: str = "BTC-USD-15m.csv") -> str:
    # Moon Dev path resolver
    moon_print(f"🌙 [Moon Dev Loader] Attempting to load data from: {preferred_path}")
    if preferred_path and os.path.isfile(preferred_path):
        moon_print("✨ [Moon Dev Loader] Found data at preferred path.")
        return preferred_path

    env_path = os.getenv("MOON_DEV_DATA_PATH", "")
    if env_path and os.path.isfile(env_path):
        moon_print(f"✨ [Moon Dev Loader] Found data at MOON_DEV_DATA_PATH: {env_path}")
        return env_path

    # Search common relative locations
    candidates = []
    cwd = os.getcwd()
    candidates.append(os.path.join(cwd, filename_hint))
    rels = [
        "data",
        "src/data",
        "src/data/rbi",
        "src/data/rbi_v2",
        "src/data/rbi_v2/10_24_2025",
        "src/data/rbi_v2/10_24_2025/backtests_package",
        "src/data/rbi_v2/10_24_2025/raw",
    ]
    for r in rels:
        candidates.append(os.path.join(cwd, r, filename_hint))

    # Glob search as last resort
    glob_patterns = [
        os.path.join(cwd, "**", filename_hint),
        os.path.join(cwd, "**", "*.csv"),
    ]
    for pat in glob_patterns:
        for p in glob.glob(pat, recursive=True):
            if os.path.basename(p).lower() == filename_hint.lower():
                candidates.append(p)

    for p in candidates:
        if os.path.isfile(p):
            moon_print(f"✨ [Moon Dev Loader] Found data at: {p}")
            return p

    raise FileNotFoundError(
        f"[Moon Dev Loader] Could not locate data file. Tried preferred path and common locations. "
        f"Set MOON_DEV_DATA_PATH or place '{filename_hint}' in ./data/"
    )


def load_data(path):
    path = _resolve_data_path(path, filename_hint=os.path.basename(path) if path else "BTC-USD-15m.csv")
    df = pd.read_csv(path)

    # Moon Dev debug
    moon_print(f"🌙 [Moon Dev Loader] Raw columns: {list(df.columns)}")

    # 1) Clean columns to lowercase and strip
    df.columns = df.columns.str.strip().str.lower()

    # 2) Drop unnamed columns
    df = df.drop(columns=[col for col in df.columns if 'unnamed' in col.lower()], errors='ignore')

    # Try to standardize datetime column if named differently
    ...
    # 3) Rename OHLCV columns to proper capitalized names later; keep lowercase for index setting
    rename_map = {}
    ...
    # 4) Set datetime as index using lowercase name per requirement
    if 'datetime' not in df.columns:
        raise ValueError("Missing required 'datetime' column after cleaning. Columns present: " + str(list(df.columns)))
    # Convert to datetime and set as index
    dt_index = pd.to_datetime(df['datetime'])
    df = df.set_index(dt_index)
    df.index.name = 'Datetime'

    # Now drop the original lowercase datetime column if still present as a column
    if 'datetime' in df.columns:
        df = df.drop(columns=['datetime'])

    # 5) Apply capitalization for OHLCV
    df = df.rename(columns=rename_map)

    # 6) Ensure required columns exist and types are numeric
    required_cols = ['Open', 'High', 'Low', 'Close', 'Volume']
    for col in required_cols:
        if col not in df.columns:
            raise KeyError(f"Missing required column after renaming: {col}")
    # Coerce to numeric
    for col in required_cols:
    ...
    after = len(df)
    if after < before:
        moon_print(f"⚠️ [Moon Dev Loader] Dropped {before - after} rows with NaNs in required columns.")

    # Final Moon Dev debug
    moon_print(f"🌙 [Moon Dev Loader] Final schema -> Index: {df.index.name}; Columns: {list(df.columns)}; Rows: {len(df)}")

    return df


if __name__ == "__main__":
    # Original preferred path
