...
def _resolve_data_path(preferred_path: str, filename_hint: str = "BTC-USD-15m.csv") -> str:
    ...
    print(f"🌙 [Moon Dev Loader] Attempting to load data from: {preferred_path}")
    if preferred_path and os.path.isfile(preferred_path):
        print("✨ [Moon Dev Loader] Found data at preferred path.")
        return preferred_path

    env_path = os.getenv("MOON_DEV_DATA_PATH", "")
    if env_path and os.path.isfile(env_path):
        print(f"✨ [Moon Dev Loader] Found data at MOON_DEV_DATA_PATH: {env_path}")
        return env_path
    ...
    for p in candidates:
        if os.path.isfile(p):
            print(f"✨ [Moon Dev Loader] Found data at: {p}")
            return p

    raise FileNotFoundError(f"❌ [Moon Dev Loader] Could not locate data file. Tried preferred path and common locations. "
...
def load_data(path):
    ...
    # Moon Dev debug
    print(f"🌙 [Moon Dev Loader] Raw columns: {list(df.columns)}")
    ...
    after = len(df)
    if after < before:
        print(f"⚠️ [Moon Dev Loader] Dropped {before - after} rows with NaNs in required columns.")

    # Final Moon Dev debug
    print(f"🌙 [Moon Dev Loader] Final schema -> Index: {df.index.name}; Columns: {list(df.columns)}; Rows: {len(df)}")
